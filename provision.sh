...
# Why this exists:
#   Windows browsers with Secure DNS / DoH (Chrome, Edge, Firefox) bypass
#   the Windows mDNS resolver entirely — *.kisaes.local fails in-browser
#   even when `ping kisaes.local` succeeds from the same box. Router DNS
#   is the "real" fix, but that requires touching every LAN router/Pi-hole.
#   This phase publishes two IP:port listeners on the host so clients can
#   always reach the apps via the LAN IP with no client-side config:
#     http://<lan-ip>:3080/  → Vibe TB   (Host rewritten to tb.<VIBE_DOMAIN>)
#     http://<lan-ip>:3081/  → Vibe MB   (Host rewritten to mb.<VIBE_DOMAIN>)
#   The Host header rewrite means backends see the canonical vhost they
#   would via the mDNS path, so CORS / cookies / absolute link construction
#   behave identically regardless of which URL the user typed.
phase_12c_ip_ports() {
    phase "PHASE 12c — IP:port fallback listeners (3080=TB, 3081=MB)"
    sudo tee /etc/nginx/sites-available/kisaes-ip-ports > /dev/null <<EOF
# Auto-generated by provision.sh. IP:port fallback for clients whose
# browser DNS bypasses mDNS (Chrome/Edge Secure DNS, Firefox DoH, etc.).
# Host header is rewritten to the canonical *.${VIBE_DOMAIN} vhost so
# backends are unaffected by the port the client hit.
...
server {
    ...
    location / {
        ...
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host tb.${VIBE_DOMAIN};
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
        proxy_read_timeout 300s;
...
server {
    ...
    location / {
        ...
        proxy_set_header Upgrade \$http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host mb.${VIBE_DOMAIN};
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
        proxy_read_timeout 300s;
...
EOF
    ...
    ok "IP:port fallback live — http://<lan-ip>:3080 (TB), http://<lan-ip>:3081 (MB)"
}
...
main() {
    ...
    if [[ "$VIBE_DOMAIN" == *.local ]]; then
        ...
        echo ""
        if [ -n "$lan_ip" ]; then
            echo "${BOLD}IP:port fallback${RESET} — for browsers with Secure DNS / DoH that bypass mDNS"
            echo "(common default on Windows Chrome / Edge / Firefox):"
            echo "  http://$lan_ip:3080/   → Vibe Trial Balance"
            echo "  http://$lan_ip:3081/   → Vibe MyBooks"
            echo "Host header is rewritten to the canonical hostname so CORS behaves correctly."
            echo ""
        fi
    else
